...
def render_comment(override_summary: nil, skip_message: nil)
  if skip_message
    body = "#{MARKER}\n### ✅ Claim validation\n\n#{skip_message}\n"
  else
    header = all_passed? ? "### ✅ Claim validation passed" : "### ❌ Claim validation failed"
    lines = @results.map do |r|
      icon = r[:ok] ? "✅" : "❌"
      detail = r[:detail].empty? ? "" : "\n    #{r[:detail].gsub("\n", "\n    ")}"
      "- #{icon} **#{r[:label]}**#{detail}"
    end
    footer = if all_passed?
...
def post_sticky_comment(body)
  repo = ENV["REPO"]
  pr = ENV["PR_NUMBER"]
  return if repo.to_s.empty? || pr.to_s.empty? || ENV["GH_TOKEN"].to_s.empty?

  _, comments = gh_api(:get, "/repos/#{repo}/issues/#{pr}/comments?per_page=100")
  existing = (comments || []).find { |c| c["body"]&.include?(MARKER) }
  if existing
    gh_api(:patch, "/repos/#{repo}/issues/comments/#{existing['id']}", { body: body })
  else
    gh_api(:post, "/repos/#{repo}/issues/#{pr}/comments", { body: body })
  end
end

def finish(skip_message: nil, override_summary: nil)
  body = render_comment(skip_message: skip_message, override_summary: override_summary)
...
def main
  ...
  finish(skip_message: "No domain claim changes detected — nothing to validate.") if domain_changes.empty?
  ...
  if %w[modified removed renamed].include?(status)
    ...
    owns = prior_owner && !PR_AUTHOR.empty? && prior_owner.downcase == PR_AUTHOR.downcase
    check(
      owns,
      "Owns the file being changed",
      owns ? "" : "Only the owner may edit or release a name. `#{path}` is owned by `#{prior_owner || 'unknown'}`, but this PR is by `#{PR_AUTHOR.empty? ? 'unknown' : PR_AUTHOR}`."
    )
  end

  # Deletions have no head file to schema-check; ownership above is the gate.
  finish if status == "removed"
  ...
  matches = !owner_github.empty? && !PR_AUTHOR.empty? && owner_github.downcase == PR_AUTHOR.downcase
  check(
    matches,
    "owner.github matches PR author",
    matches ? "" : "`owner.github` is `#{owner_github.empty? ? '(missing)' : owner_github}` but this PR is by `#{PR_AUTHOR.empty? ? 'unknown' : PR_AUTHOR}`. You can only claim a name for yourself."
  )
